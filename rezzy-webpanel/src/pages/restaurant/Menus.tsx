...
async function updateMenus(rid: string, menus: MenuItem[]) {
  const { data } = await api.put(`/restaurants/${rid}/menus`, { menus });
  return data;
}
...
export default function MenusPage() {
  ...
  const addRow = () => setRows(prev => [...prev, { name: "", price: 0, isActive: true }]);
          ...
          <div className="space-y-3">
            {rows.map((m, idx)=>(
              <div key={idx} className="grid grid-cols-1 md:grid-cols-5 gap-3 items-center">
                <input className="border rounded-lg px-3 py-2" placeholder="Ad" value={m.name}
                       onChange={(e)=> setRows(prev=> prev.map((x,i)=> i===idx?{...x, name:e.target.value}:x))}/>
                <input type="number" min={0} className="border rounded-lg px-3 py-2" placeholder="Fiyat" value={m.price}
                       onChange={(e)=> setRows(prev=> prev.map((x,i)=> i===idx?{...x, price:Number(e.target.value)||0}:x))}/>
                <label className="flex items-center gap-2 text-sm">
                  <span className="text-gray-600">Aktif</span>
                  <input type="checkbox" checked={m.isActive ?? true}
            ...
            <button className="rounded-lg bg-brand-600 hover:bg-brand-700 text-white px-4 py-2" onClick={addRow}>Yeni Menü</button>
          </div>
