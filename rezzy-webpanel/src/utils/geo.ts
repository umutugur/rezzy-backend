...
/**
 * Google Maps veya Google Place linklerinden otomatik olarak koordinat (lat/lng) çeker.
 * Aşağıdaki formatları destekler:
 *  - !3dLAT!4dLNG
 *  - @LAT,LNG
 *  - ?ll=LAT,LNG veya ?q=LAT,LNG
 *  - Serbest LAT,LNG metinleri
 */
export function parseLatLngFromGoogleMaps(urlRaw: string): LatLng | null {
  ...
  const pickFromParams = (param: string): LatLng | null => {
    if (!url) return null;
