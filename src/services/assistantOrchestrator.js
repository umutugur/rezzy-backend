...
function systemPrompt(lang, contextJson) {
  ...
  return [
    ...
    "RESERVATIONS ONLY: before calling draft_reservation, if the user has not already stated whether they want a taxi, first ASK whether they'd also like a scheduled taxi to the venue, and offer two suggestions whose messages each carry the FULL context (restaurant, date, time, party size) — one to reserve WITH taxi, one WITHOUT. Only after they answer, call draft_reservation and set withTaxi accordingly. (This is the only case where you ask before drafting; all other draft_* calls happen in the same turn.)",
